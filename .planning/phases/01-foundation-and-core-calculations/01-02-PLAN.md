...
Output: Four tested calculation modules matching Excel reference values: setupPricing, payback, co2Savings.
</objective>
...
<task type="auto">
  <name>Task 4: TDD - CO2 Savings module (RED-GREEN-REFACTOR)</name>
  <files>
    lib/calculations/__tests__/co2Savings.test.ts
    lib/calculations/co2Savings.ts
  </files>
...
Create lib/calculations/__tests__/co2Savings.test.ts:
```typescript
import Decimal from 'decimal.js';
...
Create lib/calculations/co2Savings.ts:
```typescript
import Decimal from 'decimal.js';
...
**REFACTOR:** Review for clarity, ensure JSDoc explains simplifications and assumptions.
  </action>
  <verify>
Run `npm test -- co2Savings` - all tests pass.
Run `npm test` - all calculation tests pass (setupPricing, payback, co2Savings).
  </verify>
  <done>
CO2 savings module calculates annual and lifetime CO2 reductions using Danish grid emission factor. Includes equivalent metrics (car km, trees) for user understanding. Tests verify calculations against expected values.
  </done>
</task>
...
5. CO2 savings module exports calculateCO2Savings with proper types
</verification>
...
- co2Savings.ts exports calculateCO2Savings using Danish grid emission factor
